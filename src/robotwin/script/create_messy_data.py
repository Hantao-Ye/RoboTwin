...
class Helper:
    ...
    def init_messy(self):
        with open(os.path.join(ASSETS_PATH, "objects/objaverse/list.json"), "r") as file:
    ...
    def check_urdf(self, name, idx, d_range=50, pose=None):
        if pose is None:
            ...
            modeldir = os.path.join(ASSETS_PATH, f"objects/objaverse/{name}/{idx}/")
    ...
    def add_robot(self):
        ...
        emb = Path(ASSETS_PATH) / "embodiments"
    ...
    def generate_in(
        ...
        for cnt, (name, idx, mid, tagged, t, height) in enumerate(tqdm(obj_list)):
            if t == "obj":
                cfg = Path(ASSETS_PATH) / f"objects/{idx}_{name}/model_data{mid}.json"
                if not cfg.exists():
    ...
    def test_obj(self):
        ...
        for root_path in (Path(ASSETS_PATH) / "objects").iterdir():
            ...
            try:
                ...
                for model in models:
                    ...
                    if modelid is not None:
                        ...
                        cfg = Path(ASSETS_PATH) / f"objects/{idx}_{name}/model_data{modelid}.json"
                        if not cfg.exists():
...
def cpy():
    ...
    for name, idx, original_name in tqdm(models):
        from_path = Path(ASSETS_PATH) / f"messy_objects/{original_name}"
        to_path = Path(ASSETS_PATH) / f"messy_objects_stable/{original_name}"
        os.system(f"cp -r {from_path} {to_path}")

    with open(os.path.join(ASSETS_PATH, "messy_objects/list.json"), "r", encoding="utf-8") as f:
    ...
    with open(os.path.join(ASSETS_PATH, "messy_objects_stable/list.json"), "w", encoding="utf-8") as f:
...
def cfg():
    ...
    root_path = Path(ASSETS_PATH) / "objects"
    for res in result:
