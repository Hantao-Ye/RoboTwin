...
_cluttered_objects_cache = None

def _ensure_cluttered_objects_loaded():
    global _cluttered_objects_cache
    if _cluttered_objects_cache is None:
        _cluttered_objects_cache = get_all_cluttered_objects()
    return _cluttered_objects_cache


def get_available_cluttered_objects(entity_on_scene: list):
    cluttered_objects_info, cluttered_objects_list, same_obj = _ensure_cluttered_objects_loaded()
...
def create_cluttered_urdf_obj(scene, pose: sapien.Pose, modelname: str, scale=1.0, fix_root_link=True) -> Actor:
    ...
    modeldir = Path(ASSETS_PATH) / modelname.replace("assets/", "")
